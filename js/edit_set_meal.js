...
function setRecipe(data) {
    const recipeHtml = data
        .map(recipe => {
            return `<li class="recipe" id="r-id-${recipe.id}">
                        <svg onclick="cancelRecipe(${recipe.id})" xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-x-circle cancel-recipe-btn" viewBox="0 0 16 16">
                            <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
                            <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708"/>
                        </svg>
...
async function fetchSetMeal() {
    try {
        ...
        document.getElementById("set-meal-title").value = setMealData.title;
        setTag(tagData);
...
document.getElementById("apply-edit-set-meal").addEventListener("click", async () => {
    const form = document.getElementById("edit-set-meal-form");
    ...
    try {
        const token = localStorage.getItem('jwt');
        const response = await fetch(`${backendUrl}/edit/set_meal`, {
            method: "POST",
            headers: {
                'Authorization': `Bearer ${token}`, 
                'Content-Type': 'application/json' 
...
document.getElementById("cancel-edit-set-meal").addEventListener("click", function() {
    const params = new URLSearchParams({ id, id });
    window.location.href = `set_meal.html?${params.toString()}`;
});

document.getElementById("delete-set-meal").addEventListener("click", async () => {
    try {
        const token = localStorage.getItem('jwt');
        await fetch(`${backendUrl}/delete/set_meal`, {
            method: "POST",
            headers: {
                'Authorization': `Bearer ${token}`, 
                'Content-Type': 'application/json' 
        ...
        window.location.href = `set_meals.html`;
    } catch (error) {
        console.error(`Internal server error.`, error);
    }
});
